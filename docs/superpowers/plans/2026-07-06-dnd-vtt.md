...
export function showLanding(root) {
  ...
  root.querySelector('#createBtn').onclick = async () => {
    ...
    try {
      // Creation rule validates meta/pw server-side; deleted immediately after
      // so the site password never sits readable in campaign data.
...
export function showRolePopup(root, cid, onRole) {
  ...
  function renderList() {
    ...
    for (const [id, ch] of Object.entries(chars)) {
      ...
      const label = ch.name + (ch.hidden ? ' (inactive)' : '') + (ch.speed ? ` · ${ch.speed} ft` : '');
...
export function openCalibration(rail, mapId, map) {
      ...
      if (mode === 'tune') {
        if (clicks.length === 2) {
          ...
          p.querySelector('#calGo').onclick = () => {
            grid = { ...grid, ...calibrate(clicks[0], clicks[1], Number(p.querySelector('#calN').value)) };
            clicks = [];
            redraw(); renderTune(p);
          };
        } else renderTune(p);
  ...
  function renderTune(p) {
    p.innerHTML = `<h3>Fine-tune grid</h3>
      <label>Cell size (px)</label><input id="tCell" type="number" step="0.1" value="${grid.cellPx.toFixed(1)}">
    ...
    bind('#tCell', 'cellPx'); bind('#tOffX', 'offX'); bind('#tOffY', 'offY');
...
export function createTokenLayer(worldEl) {
  ...
  function renderCharacters(chars) {
    ...
    for (const [id, ch] of Object.entries(chars || {})) {
      ...
      el.addEventListener('mouseenter', e => showTip(e, ch.name));
...
```js
  ...
  store.sub(`campaigns/${cid}/characters`, chars => {
...
```js
  ...
  store.sub(`campaigns/${cid}/drags`, v => { remoteDrags = v || {}; redrawRulers(); });
    ...
    for (const [id, m] of Object.entries(monsters || {})) {
      ...
      el.addEventListener('mouseenter', e => {
        if (isDm || m.hpVisible) {
          ...
          showTip(e, `${lib.label} · ${m.hp.cur}/${m.hp.max}<div class="hpbar"><div style="width:${pct}%"></div></div>`);
        } else showTip(e, lib.label);
      });
      el.addEventListener('mouseleave', hideTip);
...
export function createMonsterTool(rail, getLib) {
  ...
  function hpPanel(id) {
    ...
    store.readOnce(base).then(m => {
      ...
      const lib = library()[m.libRef];
      rail.showPopover(p => {
        p.innerHTML = `<h3>${lib?.label || 'Monster'} HP</h3>
...
```js
  ...
  store.sub(`campaigns/${cid}/monsterLibrary`, v => {
    ctx.monsterLib = v || {};
...
```js
    ...
    for (const [id, ch] of Object.entries(chars)) {
      const row = document.createElement('div');
      row.innerHTML = `<span>${ch.name}${ch.hidden ? ' (hidden)' : ''}</span>
...
export function showCharacterPanel(rail, charId) {
  ...
  rail.showPopover(p => {
    p.innerHTML = `<h3>Your character</h3>
      <label>Name</label><input id="cpName" value="${ch.name}">
...
```js
  ...
  store.onConnectionChange(ok => banner.style.display = ok ? 'none' : 'block');
```
